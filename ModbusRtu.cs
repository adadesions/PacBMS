...
namespace DAL
{
    public class ModbusRtu
    {
        ...
        public bool PreSetKeepReg(int iDevAdd, int iAddress, int SetValue)
        {
            ...
            return ByteArrayEquals(ResByte,SendCommand);
        }
        #endregion
        ...
        /// <param name="SetValue"></param>
        /// <returns></returns>
        public bool PreSetMultiKeepReg(int iDevAdd, int iAddress, byte[] bSetValue)
        {
            byte[] ResByte;
            byte[] SendCommand = new byte[137];
            CurrentAddr = iDevAdd;
            SendCommand[0] = (byte)iDevAdd;
            SendCommand[1] = 0x10;
            SendCommand[2] = (byte)((iAddress - iAddress % 256) / 256);
            ...
            for (int i = 0; i < 128; i++)
            {
                SendCommand[i + 7] = bSetValue[i];
            }
            Crc16(SendCommand, 135);
            SendCommand[135] = ucCRCLo;
            SendCommand[136] = ucCRCHi;


            try
            {
                MyCom.Write(SendCommand, 0, 137);
            }
            catch (Exception)
            {
                return false;
            ...
            byte[] byteTemp = GetByteArray(ResByte, 4, 2);

            int next = FromByteArray(byteTemp);
        ...
        private Int16 FromByteArray(byte[] b)
        {
            short s = 0;
